...
### 2026-07-09 17:57
- User asked: continue sustained optimization toward the repo goal, focusing only on code/system quality and preserving the full-test-before-upload rule.
- Work completed: researched agent-memory work emphasizing auditable/governed memory operations; audited SDK import/export and batch paths after REST/OpenAI/LangChain/MCP parameter parity work; found `import_memories()` only forwarded `metadata`, dropping `source`, `source_file`, `ttl`, and `tags`; fixed imports to pass those fields into `remember()`; updated the docstring; added regression tests for list imports preserving metadata/source/ttl/tags and export-style `source_file`.
- Files touched: `src/mempalace_evolve/sdk.py`, `tests/test_batch_ops.py`, `.codex-project/PROJECT_MEMORY.md`; pre-existing `README.md` example change remains unstaged.
- Verification: targeted `tests/test_batch_ops.py tests/test_export.py tests/test_new_features_v2.py` passed `58 passed`; full suite passed `648 passed, 7 skipped, 1 warning in 67.85s`; `git diff --check` passed with CRLF warnings only; generated-file scan for adaptive baselines/sqlite artifacts returned no source/test pollution.
- Result: memory migration/import now preserves lifecycle and access-label fields, keeping ingestion semantics aligned across SDK, batch, REST, MCP, OpenAI, and LangChain entry points.
- Next suggested move: commit and push this import metadata parity pass, then inspect batch operation surfaces, export fidelity, or terminal-agent runtime polish.

### 2026-07-09 17:41
- User asked: continue sustained optimization toward the repo goal, focusing only on code/system quality and preserving the full-test-before-upload rule.
- Work completed: researched recent agent-memory framing around governed/auditable memory operations; audited REST `/remember` against SDK/MCP/OpenAI/LangChain typed write parameters; added `ttl` and `tags` to `RememberRequest`; forwarded them to `MemPalace.remember()`; added a REST adapter regression test that writes metadata/source/ttl/tags through HTTP and verifies Chroma metadata contains `priority`, `source_file`, `tags`, and future `expire_at`.
- Files touched: `src/mempalace_evolve/adapters/rest_api.py`, `tests/test_adapters.py`, `.codex-project/PROJECT_MEMORY.md`; pre-existing `README.md` example change remains unstaged.
- Verification: targeted `tests/test_adapters.py tests/test_new_features_v2.py tests/test_sdk.py` passed `54 passed, 7 skipped, 1 warning`; full suite passed `646 passed, 7 skipped, 1 warning in 66.79s`; `git diff --check` passed with CRLF warnings only; generated-file scan for adaptive baselines/sqlite artifacts returned no source/test pollution.
- Result: SDK, MCP, OpenAI, LangChain, and REST write paths now all support metadata/source/ttl/tags, improving cross-entry memory ingestion consistency.
- Next suggested move: commit and push this REST typed-parameter parity pass, then inspect batch/import/export metadata parity or terminal-agent runtime polish.
...
- Verification: CLI/server targeted tests passed 5/5; CI-equivalent coverage passed 649 passed and 11 skipped; full Ruff check and format check passed; package build and twine checks passed; SDK import and doctor passed 4/4; README diff was empty; remote `Tests` and `CI` for `3a58661` both completed successfully, and the new `5380e4c` `Tests` and `CI` runs also both completed successfully.
...
### 2026-07-18 (memory quality, API parity, and documentation pass)
- User asked to resolve the audited README, documentation, memory-quality, lifecycle, validation, cross-adapter, REST reliability, observability, and evaluation gaps. The DreamSeed Contest template and top module in `README.md` remain unchanged.
- Work completed: rewrote the MemPalace README section and the API reference, quick start, index, architecture text, and runnable examples around the actual SDK/CLI/REST/MCP contracts. Added README documentation smoke tests and a fixed retrieval-quality fixture for expected recall and cross-wing isolation.
- Core changes: recall results now include `drawer_id` plus an `explanation` payload (source, time, score components, match reason, lifecycle state, and replacement context); expired TTL memories are excluded at recall time; `stats()` reports active/stale/superseded/expired lifecycle counts; contradiction detection now requires textual overlap in addition to vector proximity; SDK validates write/query/batch boundaries while retaining the legacy empty-query `[]` behavior.
- Adapter changes: REST now validates body limits and forwards `threshold`/`hybrid`; its Windows smoke test no longer silently skips due to temporary-directory cleanup. MCP, OpenAI, and LangChain recall paths now forward `threshold`/`hybrid`; MCP and LangChain preserve the full explainable result structure.
- Verification: targeted quality/docs/REST tests passed 17/17; adapters and async tests passed 54 with 2 optional LangChain skips; full CI-equivalent suite passed `673 passed, 2 skipped, 1 warning` with coverage; `ruff check src` and `ruff format --check src` passed; package build and `twine check` passed; `git diff --check` passed (only existing CRLF conversion warnings). A full `ruff check tests` finds 59 pre-existing test lint errors, intentionally not mass-refactored in this focused pass.
- Next suggested move: push the verified pass, inspect both remote GitHub Actions workflows, then move to the remaining larger reliability work: a process-level concurrent-write recovery test and explicit knowledge-graph relationship export/import.
