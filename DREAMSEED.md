...
DreamSeed runs on the bundled DreamSeed Lite Kernel
(`bin/dreamseed-lite-kernel.js`). No additional runtime configuration is
required. To start an interactive session:
...
You can override the kernel path with `DREAMSEED_KERNEL_JS` when developing on
a different kernel build, but most users never need to.
