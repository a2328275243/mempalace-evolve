...
After publishing, set `DREAMSEED_COMPAT_KERNEL_JS` to a local compatible JavaScript kernel or set `DREAMSEED_COMPAT_KERNEL_CLI` to an installed compatible command, then run `dreamseed`. If a model provider is needed, set `DREAMSEED_PROVIDER_CONFIG` to a private provider config; DreamSeed will start its own provider bridge.
