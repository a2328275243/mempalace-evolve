  ...
  buildTool({
    ...
    async checkPermissions(input, _context) {
      if (feature('UDS_INBOX') && parseAddress(input.to).scheme === 'bridge') {
        return {
          ...
          message: `Send a message to Remote Control session ${input.to}? It arrives as a user prompt on the receiving DreamSeed (possibly another machine) via Anthropic's servers.`,
    ...
    async validateInput(input, _context) {
      ...
      if (feature('UDS_INBOX') && parseAddress(input.to).scheme === 'bridge') {
        ...
        // postInterDreamSeedMessage derives from= via getReplBridgeHandle() —
