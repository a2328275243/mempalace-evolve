...
function isPathAllowed(
  ...
  // This MUST come before checkPathSafetyForAutoEdit since .dreamseed is a dangerous directory
  // and internal editable paths live under ~/.dreamseed/ — matching the ordering in
...
function checkPathConstraintsForStatement(
  ...
  //   Set-Location ./.dreamseed; Set-Content ./settings.json '...'
  // Validator sees ./settings.json → /project/settings.json (not a config file).
  // Runtime writes /project/.dreamseed/settings.json (DreamSeed's permission config).
  //
  // ALTERNATIVE APPROACH (rejected): simulate cwd through the statement chain
  // — after `Set-Location ./.dreamseed`, validate subsequent statements with
  // cwd='./.dreamseed'. This would be more permissive but requires careful
  // handling of:
      ...
      if (!allowed) {
        ...
        const message =
          ...
          decisionReason?.type === 'safetyCheck'
            ...
            : `${canonical} targeting '${resolvedPath}' was blocked. For security, DreamSeed Code may only access files in the allowed working directories for this session: ${dirListStr}.`
