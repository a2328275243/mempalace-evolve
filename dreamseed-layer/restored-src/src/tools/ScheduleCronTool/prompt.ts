...
export function buildCronCreateDescription(durableEnabled: boolean): string {
  return durableEnabled
    ? 'Schedule a prompt to run at a future time — either recurring on a cron schedule, or once at a specific time. Pass durable: true to persist to .dreamseed/scheduled_tasks.json; otherwise session-only.'
    : 'Schedule a prompt to run at a future time within this DreamSeed session — either recurring on a cron schedule, or once at a specific time.'
}
...
By default (durable: false) the job lives only in this DreamSeed session — nothing is written to disk, and the job is gone when DreamSeed exits. Pass durable: true to write to .dreamseed/scheduled_tasks.json so the job survives restarts. Only use durable: true when the user explicitly asks for the task to persist ("keep doing this every day", "set this up permanently"). Most "remind me in 5 minutes" / "check back in an hour" requests should stay session-only.`
    : `## Session-only

Jobs live only in this DreamSeed session — nothing is written to disk, and the job is gone when DreamSeed exits.`

  const durableRuntimeNote = durableEnabled
    ? 'Durable jobs persist to .dreamseed/scheduled_tasks.json and survive session restarts — on next launch they resume automatically. One-shot durable tasks that were missed while the REPL was closed are surfaced for catch-up. Session-only jobs die with the process. '
    : ''
...
export function buildCronDeletePrompt(durableEnabled: boolean): string {
  return durableEnabled
    ? `Cancel a cron job previously scheduled with ${CRON_CREATE_TOOL_NAME}. Removes it from .dreamseed/scheduled_tasks.json (durable jobs) or the in-memory session store (session-only jobs).`
...
export function buildCronListPrompt(durableEnabled: boolean): string {
  return durableEnabled
    ? `List all cron jobs scheduled via ${CRON_CREATE_TOOL_NAME}, both durable (.dreamseed/scheduled_tasks.json) and session-only.`
