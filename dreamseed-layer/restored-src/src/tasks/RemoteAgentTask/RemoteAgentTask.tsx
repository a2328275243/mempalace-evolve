...
export function formatPreconditionError(error: BackgroundRemoteSessionPrecondition): string {
  switch (error.type) {
    case 'not_logged_in':
      return 'Please run /login and sign in with your DreamSeed.ai account (not Console).';
 ...
 *   {type:'system', subtype:'hook_progress', stdout:'...'}. DreamSeed never
...
function startRemoteSessionPolling(taskId: string, context: TaskContext): () => void {
  ...
  const poll = async (): Promise<void> => {
    ...
    try {
      ...
      // stableIdle is a prompt-mode completion signal (DreamSeed stops writing
