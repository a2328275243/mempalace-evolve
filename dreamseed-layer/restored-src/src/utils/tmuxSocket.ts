 ...
 * =====================
 * This module manages an isolated tmux socket for DreamSeed's operations.
 *
 * WHY THIS EXISTS:
 * Without isolation, DreamSeed could accidentally affect the user's tmux sessions.
 * For example, running `tmux kill-session` via the Bash tool would kill the
 * user's current session if they started DreamSeed from within tmux.
 *
 * HOW IT WORKS:
 * 1. DreamSeed creates its own tmux socket: `claude-<PID>` (e.g., `claude-12345`)
 ...
 *    (set in Shell.ts via getDreamSeedTmuxEnv())
 *
 * This means ANY tmux command run through DreamSeed - whether via the Tmux tool
 * directly or via Bash - will operate on DreamSeed's isolated socket, NOT the
 * user's tmux session.
 *
 * IMPORTANT: The user's original TMUX env var is NOT used. After socket
 * initialization, getDreamSeedTmuxEnv() returns a value that overrides the
...
/**
 * Gets the socket name for DreamSeed's isolated tmux session.
 * Format: claude-<PID>
 */
...
/**
 * Gets the TMUX environment variable value for DreamSeed's isolated socket.
 *
 ...
 * the Bash tool will operate on DreamSeed's socket, NOT the user's session.
 *
 ...
 * initializing the socket. getDreamSeedTmuxEnv() will return null, and
 * Bash commands will run without tmux isolation.
 */
...
/**
 * Kills the tmux server for DreamSeed's isolated socket.
...
async function doInitialize(): Promise<void> {
  ...
  // Any DreamSeed Code instance spawned on this socket will inherit this env var,
