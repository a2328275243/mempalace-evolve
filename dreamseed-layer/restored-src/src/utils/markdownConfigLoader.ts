...
// DreamSeed configuration directory names
export const DREAMSEED_CONFIG_DIRECTORIES = [
  'commands',
  'agents',
  'output-styles',
  'skills',
...
export type ClaudeConfigDirectory = (typeof DREAMSEED_CONFIG_DIRECTORIES)[number]
 ...
 * stopping there makes the parent project's `.dreamseed/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.dreamseed/worktrees/`) stay on the old behavior: their `.git`
 ...
 * collecting all .dreamseed directories along the way.
 *
 * Stopping at git root prevents commands/skills from parent directories outside the repository
 * from leaking into projects. For example, if ~/projects/.dreamseed/commands/ exists, it won't
 ...
 * @returns Array of directory paths containing .dreamseed/subdir, from most specific (cwd) to least specific
 */
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const claudeSubdir = join(current, '.dreamseed', subdir)
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const managedDir = join(getManagedFilePath(), '.dreamseed', subdir)
    const projectDirs = getProjectDirsUpToHome(subdir, cwd)

    // For git worktrees where the worktree does NOT have .dreamseed/<subdir> checked
    ...
    //
    // Only add the main repo's copy when the worktree root's .dreamseed/<subdir>
    // is absent. A standard `git worktree add` checks out the full tree, so the
    // worktree already has identical .dreamseed/<subdir> content — loading the main
    ...
    // This prevents the same file from appearing multiple times when ~/.dreamseed is
  ...
  (subdir: DreamSeedConfigDirectory, cwd: string) => `${subdir}:${cwd}`,
)
 ...
 * @param dir Directory (eg. "~/.dreamseed/commands")
 * @returns Array of parsed markdown files with metadata
 */
