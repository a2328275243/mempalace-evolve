...
/**
 * If filePath is inside a .dreamseed/skills/{name}/ directory (project or global),
 ...
 * require granting session access to all of .dreamseed/ (settings.json, hooks/, etc.).
 */
export function getClaudeSkillScope(
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.dreamseed', 'skills')),
      prefix: '/.dreamseed/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.dreamseed', 'skills')),
      prefix: '~/.dreamseed/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.dreamseed/skills/*/**' which matches ALL skills. Return null
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  // sequences like `./.dreamseed/./settings.json` which would evade the endsWith() check
  ...
  if (
    normalizedPath.endsWith(`${sep}.dreamseed${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.dreamseed${sep}settings.local.json`)
  ) {
    // Include .dreamseed/settings.json even for other projects
    return true
  }
...
// Always ask when DreamSeed Code tries to edit its own config files
function isClaudeConfigFilePath(filePath: string): boolean {
  if (isClaudeSettingsPath(filePath)) {
    return true
  }

  // Check if file is within .dreamseed/commands or .dreamseed/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.dreamseed', 'commands')
  const agentsDir = join(getOriginalCwd(), '.dreamseed', 'agents')
  const skillsDir = join(getOriginalCwd(), '.dreamseed', 'skills')
 ...
 * Path format: ~/.dreamseed/projects/{sanitized-cwd}/...
 */
 ...
 * The scratchpad is a per-session directory for DreamSeed to write temporary files.
...
/**
 * Returns the user-specific DreamSeed temp directory name.
...
/**
 * Returns the DreamSeed temp directory path with symlinks resolved.
 ...
 * This is a per-user temporary directory used by DreamSeed Code for all temp files.
 *
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .dreamseed/worktrees/ is a structural path (where DreamSeed stores
      // git worktrees), not a user-created dangerous directory. Skip the .dreamseed
      // segment when it's followed by 'worktrees'. Any nested .dreamseed directories
      ...
      if (dir === '.dreamseed') {
        ...
        if (
          ...
          break // Skip this .dreamseed, continue checking other segments
        }
      }
 ...
 * - Trailing dots and spaces (e.g., .git., .dreamseed , .bashrc...)
...
function hasSuspiciousWindowsPathPattern(path: string): boolean {
  ...
  // Examples: .git., .dreamseed , .bashrc..., settings.json.
 ...
 * - DreamSeed config files (.dreamseed/settings.json, .dreamseed/commands/, .dreamseed/agents/)
 * - MCP CLI state files (managed internally by DreamSeed Code)
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `DreamSeed requested permissions to write to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        classifierApprovable: false,
      }
    }
  }

  // Check for DreamSeed config files on all paths
  for (const pathToCheck of pathsToCheck) {
    if (isClaudeConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `DreamSeed requested permissions to write to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `DreamSeed requested permissions to edit ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .dreamseed/)
    return {
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `DreamSeed requested permissions to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `DreamSeed requested permissions to read from ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `DreamSeed requested permissions to read from ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `DreamSeed requested permissions to read from ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .dreamseed is a dangerous directory
  ...
  // 1.6. Check for .dreamseed/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .dreamseed/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .dreamseed/ folder.
  //
  // matchingRuleForInput returns the first match across all sources. If the user
  // also has a broader Edit(.dreamseed) rule in userSettings (e.g. from sandbox
  ...
  // "allow DreamSeed to edit its own settings for this session" option actually works.
  ...
  if (claudeFolderAllowRule) {
    // Check if this rule is scoped under .dreamseed/ (project or global).
    // Accepts both the broad patterns ('/.dreamseed/**', '~/.dreamseed/**') and
    // narrowed ones like '/.dreamseed/skills/my-skill/**' so users can grant
    ...
    // '/.dreamseed/../**' from leaking this bypass outside .dreamseed/.
  ...
  // 1.7. Check comprehensive safety validations (Windows patterns, DreamSeed config, dangerous files)
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .dreamseed/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.dreamseed/settings.json, .git/, .vscode/, .idea/) falls
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.dreamseed/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      ...
      // the case where ~/.dreamseed is a symlink (e.g. to /data/claude-config).
  ...
  // ~/.dreamseed/, which is in DANGEROUS_DIRECTORIES. The DREAMSEED_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .dreamseed/launch.json — desktop preview config (dev server command + port).
  // The desktop's preview_start MCP tool instructs DreamSeed to create/update
  // this file as part of the preview workflow. Without this carve-out the
  // .dreamseed/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .dreamseed/ only (not ~/.dreamseed/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.dreamseed', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Tasks directory (~/.dreamseed/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.dreamseed/teams/) for swarm coordination
