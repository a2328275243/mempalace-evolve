...
export async function fetchEnvironments(): Promise<EnvironmentResource[]> {
  ...
  if (!accessToken) {
    throw new Error(
      'DreamSeed Code web sessions require authentication with a DreamSeed.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.',
    )
  }
