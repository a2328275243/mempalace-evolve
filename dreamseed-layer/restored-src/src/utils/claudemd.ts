 ...
 *
 * 1. Managed memory (eg. /etc/claude-code/DREAMSEED.md) - Global instructions for all users
 * 2. User memory (~/.dreamseed/DREAMSEED.md) - Private global instructions for all projects
 * 3. Project memory (DREAMSEED.md, .dreamseed/DREAMSEED.md, and .dreamseed/rules/*.md in project roots) - Instructions checked into the codebase
 * 4. Local memory (DREAMSEED.local.md in project roots) - Private project-specific instructions
 *
 ...
 * - DREAMSEED.md, .dreamseed/DREAMSEED.md, and all .md files in .dreamseed/rules/ are checked in each directory for Project memory
 *
...
/**
 * Checks whether a DREAMSEED.md file path is excluded by the dreamseedMdExcludes setting.
...
function isClaudeMdExcluded(filePath: string, type: MemoryType): boolean {
  ...
  const patterns = getInitialSettings().dreamseedMdExcludes
  ...
  // the user writes "/tmp/project/DREAMSEED.md" in their exclude, but the system
...
function resolveExcludePatterns(patterns: string[]): string[] {
  ...
  for (const normalized of expanded) {
    ...
    try {
      // sync IO: called from sync context (isDreamSeedMdExcluded -> processMemoryFile -> getMemoryFiles)
...
export async function processMemoryFile(
  ...
  // Skip if path is excluded by dreamseedMdExcludes setting
...
/**
 * Processes all .md files in the .dreamseed/rules/ directory and its subdirectories
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // Process Managed .dreamseed/rules/*.md files
    ...
    if (isSettingSourceEnabled('userSettings')) {
      ...
      // Process User ~/.dreamseed/rules/*.md files
    ...
    // .dreamseed/worktrees/<name>/ from `claude -w`), the upward walk passes
    // through both the worktree root and the main repo root. Both contain
    // checked-in files like DREAMSEED.md and .dreamseed/rules/*.md, so the same
    // content gets loaded twice. Skip Project-type (checked-in) files from
    // directories above the worktree but within the main repo — the worktree
    // already has its own checkout. DREAMSEED.local.md is gitignored so it only
    ...
    for (const dir of dirs.reverse()) {
      ...
      // Try reading DREAMSEED.md (Project) - only if projectSettings is enabled
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        const projectPath = join(dir, 'DREAMSEED.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
            'Project',
        ...
        // Try reading .dreamseed/DREAMSEED.md (Project)
        const dotClaudePath = join(dir, '.dreamseed', 'DREAMSEED.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
            'Project',
        ...
        // Try reading .dreamseed/rules/*.md files (Project)
        const rulesDir = join(dir, '.dreamseed', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
            type: 'Project',
      ...
      // Try reading DREAMSEED.local.md (Local) - only if localSettings is enabled
      if (isSettingSourceEnabled('localSettings')) {
        const localPath = join(dir, 'DREAMSEED.local.md')
        result.push(
          ...(await processMemoryFile(
            localPath,
            'Local',
    ...
    // Process DREAMSEED.md from additional directories (--add-dir) if env var is enabled
    ...
    if (isEnvTruthy(process.env.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
      ...
      for (const dir of additionalDirs) {
        // Try reading DREAMSEED.md from the additional directory
        const projectPath = join(dir, 'DREAMSEED.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
        ...
        // Try reading .dreamseed/DREAMSEED.md from the additional directory
        const dotClaudePath = join(dir, '.dreamseed', 'DREAMSEED.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
        ...
        // Try reading .dreamseed/rules/*.md files from the additional directory
        const rulesDir = join(dir, '.dreamseed', 'rules')
        result.push(
          ...(await processMdRules({
            rulesDir,
    ...
    // memory system, not "instructions" in the DREAMSEED.md/rules sense.
    // Gated on !forceIncludeExternal: the forceIncludeExternal=true variant
    // is only used by getExternalDreamSeedMdIncludes() for approval checks, not
...
export async function getManagedAndUserConditionalRules(
  ...
  // Process Managed conditional .dreamseed/rules/*.md files
  const managedClaudeRulesDir = getManagedClaudeRulesDir()
  result.push(
  ...
  if (isSettingSourceEnabled('userSettings')) {
    // Process User conditional .dreamseed/rules/*.md files
    const userClaudeRulesDir = getUserClaudeRulesDir()
    result.push(
      ...(await processConditionedMdRules(
        targetPath,
 ...
 * Loads DREAMSEED.md, unconditional rules, and conditional rules for that directory.
 *
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files (DREAMSEED.md and .dreamseed/DREAMSEED.md)
  ...
  // Process local memory file (DREAMSEED.local.md)
  if (isSettingSourceEnabled('localSettings')) {
    const localPath = join(dir, 'DREAMSEED.local.md')
    result.push(
  ...
  // Process project unconditional .dreamseed/rules/*.md files, which were not eagerly loaded
  ...
  // Process project conditional .dreamseed/rules/*.md files
  result.push(
    ...(await processConditionedMdRules(
      targetPath,
      rulesDir,
...
export async function processConditionedMdRules(
  ...
  return conditionedRuleMdFiles.filter(file => {
    ...
    // For Project rules: glob patterns are relative to the directory containing .dreamseed
    // For Managed/User rules: glob patterns are relative to the original CWD
    const baseDir =
      type === 'Project'
        ? dirname(dirname(rulesDir)) // Parent of .dreamseed
...
/**
 * Check if a file path is a memory file (DREAMSEED.md, DREAMSEED.local.md, or .dreamseed/rules/*.md)
 */
export function isMemoryFilePath(filePath: string): boolean {
  const name = basename(filePath)

  // DREAMSEED.md or DREAMSEED.local.md anywhere
  if (name === 'DREAMSEED.md' || name === 'DREAMSEED.local.md') {
    return true
  }

  // .md files in .dreamseed/rules/ directories
  if (
    name.endsWith('.md') &&
    filePath.includes(`${sep}.dreamseed${sep}rules${sep}`)
  ) {
    return true
  }
