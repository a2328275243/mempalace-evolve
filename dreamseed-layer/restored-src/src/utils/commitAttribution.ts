...
/**
 * Attribution state for tracking DreamSeed's contributions to files.
 */
...
/**
 * Summary of DreamSeed's contribution for a commit.
 */
...
function computeFileModificationState(
  ...
  try {
    // Calculate DreamSeed's character contribution
    ...
    const existingContribution = existingState?.dreamseedContribution ?? 0
...
/**
 * Track a file modification by DreamSeed.
...
export function trackFileModification(
  ...
  logForDebugging(
    `Attribution: Tracked ${newFileState.dreamseedContribution} chars for ${normalizedPath}`,
  )
...
/**
 * Track a file creation by DreamSeed (e.g., via bash command).
 * Used when DreamSeed creates a new file through a non-tracked mechanism.
 */
...
/**
 * Track a file deletion by DreamSeed (e.g., via bash rm command).
 * Used when DreamSeed deletes a file through a non-tracked mechanism.
 */
export function trackFileDeletion(
  ...
  logForDebugging(
    `Attribution: Tracked deletion of ${normalizedPath} (${deletedChars} chars removed, total contribution: ${newFileState.dreamseedContribution})`,
  )
...
export async function calculateCommitAttribution(
    ...
    for (const [path, fileState] of fileStates) {
      ...
      if (existing) {
        mergedFileStates.set(path, {
          ...fileState,
          claudeContribution:
            existing.dreamseedContribution + fileState.dreamseedContribution,
        })
      } else {
  ...
  const fileResults = await Promise.all(
    stagedFiles.map(async file => {
      ...
      if (deleted) {
        // File was deleted
        if (fileState) {
          // DreamSeed deleted this file (tracked deletion)
          claudeChars = fileState.dreamseedContribution
          humanChars = 0
        } else {
        ...
        try {
          ...
          if (fileState) {
            ...
            // New file not created by DreamSeed
  ...
  for (const result of fileResults) {
    ...
    files[result.file] = {
      claudeChars: result.dreamseedChars,
      humanChars: result.humanChars,
      percent: result.percent,
      surface: result.surface,
    }

    totalClaudeChars += result.dreamseedChars
    totalHumanChars += result.humanChars

    surfaceCounts[result.surface] =
      (surfaceCounts[result.surface] ?? 0) + result.dreamseedChars
  }
