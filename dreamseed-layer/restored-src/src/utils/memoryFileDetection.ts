...
/**
 * Detects if a file path is a session-related file under ~/.dreamseed.
...
/**
 * Check if a file is a DreamSeed-managed memory file (NOT user-managed instruction files).
 * Includes: auto-memory (memdir), agent memory, session memory/transcripts.
 * Excludes: DREAMSEED.md, DREAMSEED.local.md, .dreamseed/rules/*.md (user-managed).
 *
...
export function isMemoryDirectory(dirPath: string): boolean {
  ...
  // Check the auto-memory path override (DREAMSEED_COWORK_MEMORY_PATH_OVERRIDE)
  if (isAutoMemoryEnabled()) {
...
// Excludes DREAMSEED.md, DREAMSEED.local.md, .dreamseed/rules/ (user-managed).
