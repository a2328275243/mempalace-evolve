      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[DreamSeed Browser] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[DreamSeed Browser] No compatible browser found')
    return false
  }
...
export function getAllSocketPaths(): string[] {
  ...
  try {
    // eslint-disable-next-line custom-rules/no-sync-fs -- DreamSeedForChromeContext.getSocketPaths (external @ant/claude-for-chrome-mcp) requires a sync () => string[] callback
...
function getSocketName(): string {
  // NOTE: This must match the one used in the DreamSeed Browser MCP
