// Scheduler lease lock for .dreamseed/scheduled_tasks.json.
//
// When multiple DreamSeed sessions run in the same project directory, only one
...
const LOCK_FILE_REL = join('.dreamseed', 'scheduled_tasks.lock')
...
async function tryCreateExclusive(
  ...
  try {
    ...
    if (code === 'ENOENT') {
      // .dreamseed/ doesn't exist yet — create it and retry once. In steady
