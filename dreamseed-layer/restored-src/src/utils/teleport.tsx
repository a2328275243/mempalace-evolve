...
/**
 * Generates a title and branch name for a coding session using DreamSeed Haiku
...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    ...
    if (!accessToken) {
      ...
      throw new Error('DreamSeed Code web sessions require authentication with a DreamSeed.ai account. API key authentication is not sufficient. Please run /login to authenticate, or check your authentication status with /status.');
    }
