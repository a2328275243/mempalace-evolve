...
/**
 * Generates DreamSeed Code specific snapshot content
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in DreamSeed's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and DreamSeed Code content
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with DreamSeed Code defaults only`,
        )
      }
      ...
      execFile(
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - DreamSeed home: ${getClaudeConfigHomeDir()}`)
