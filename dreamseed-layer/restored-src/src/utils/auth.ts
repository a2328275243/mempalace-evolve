...
/**
 * CCR and DreamSeed Desktop spawn the CLI with OAuth and should never fall back
 * to the user's ~/.dreamseed/settings.json API-key config (apiKeyHelper,
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // `dreamseed ssh` remote: ANTHROPIC_UNIX_SOCKET tunnels API calls through a
  ...
  // ~/.dreamseed settings (apiKeyHelper, settings.env.ANTHROPIC_API_KEY) MUST NOT
 ...
 * from ~/.dreamseed/settings.json or project settings is ignored.
 */
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running awsAuthRefresh (in settings or ~/.dreamseed.json):',
            )
  ...
  try {
    ...
    try {
      ...
      const message = chalk.red(
        'Error getting AWS credentials from awsCredentialExport (in settings or ~/.dreamseed.json):',
      )
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running gcpAuthRefresh (in settings or ~/.dreamseed.json):',
            )
        // biome-ignore lint/suspicious/noConsole:: intentional console output
        console.error(message)
        authStatusManager.endAuthentication(false)
...
export function saveOAuthTokensIfNeeded(tokens: OAuthTokens): {
  ...
  try {
    ...
    const existingOauth = storageData.dreamseedAiOauth

    storageData.dreamseedAiOauth = {
...
export const getClaudeAIOAuthTokens = memoize((): OAuthTokens | null => {
  ...
  try {
    const secureStorage = getSecureStorage()
    const storageData = secureStorage.read()
    const oauthData = storageData?.dreamseedAiOauth
...
export function isOverageProvisioningAllowed(): boolean {
  ...
  // Must be a DreamSeed subscriber with a supported subscription type
...
export function getSubscriptionName(): string {
  ...
  switch (subscriptionType) {
    case 'enterprise':
      return 'DreamSeed Enterprise'
    case 'team':
      return 'DreamSeed Team'
    case 'max':
      return 'DreamSeed Max'
    case 'pro':
      return 'DreamSeed Pro'
    default:
      return 'DreamSeed API'
  }
}
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  // `dreamseed ssh` remote: real auth lives on the local machine and is injected
  ...
  // in ~/.dreamseed.json is user-writable and cannot be trusted.
