/**
 * MDM (Mobile Device Management) profile enforcement for DreamSeed Code managed settings.
 *
 * Reads enterprise settings from OS-level MDM configuration:
 * - macOS: `com.anthropic.dreamseedcode` preference domain
 *   (MDM profiles at /Library/Managed Preferences/ only — not user-writable ~/Library/Preferences/)
 * - Windows: `HKLM\SOFTWARE\Policies\DreamSeedCode` (admin-only)
 *   and `HKCU\SOFTWARE\Policies\DreamSeedCode` (user-writable, lowest priority)
