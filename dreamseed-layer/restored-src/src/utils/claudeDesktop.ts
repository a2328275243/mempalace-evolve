...
export async function getClaudeDesktopConfigPath(): Promise<string> {
  ...
  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - DreamSeed Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  try {
    ...
    try {
      ...
      // Look for DreamSeed Desktop config in each user directory
  ...
  throw new Error(
    'Could not find DreamSeed Desktop config file in Windows. Make sure DreamSeed Desktop is installed on Windows.',
  )
}
...
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - DreamSeed Desktop integration only works on macOS and WSL.',
    )
  }
  try {
