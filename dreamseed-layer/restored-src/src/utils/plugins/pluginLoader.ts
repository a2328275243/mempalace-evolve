 ...
 *
 * This module is responsible for discovering, loading, and validating DreamSeed Code plugins
 ...
 * Format: ~/.dreamseed/plugins/cache/{marketplace}/{plugin}/{version}/
 *
 * @param pluginId - Plugin identifier in format "name@marketplace"
 * @param version - Version string (semver, git SHA, etc.)
 ...
 * Format: ~/.dreamseed/plugins/cache/{plugin-name}/
 *
...
export async function cachePlugin(
  ...
  const manifestPath = join(tempPath, '.dreamseed-plugin', 'plugin.json')
 ...
 * @param source - Source description for default manifest (e.g., "git:repo" or ".dreamseed-plugin/name")
 ...
 * @param source - Source identifier (e.g., "git:repo", ".dreamseed-plugin/my-plugin")
...
export async function createPluginFromPath(
  ...
  const manifestPath = join(pluginPath, '.dreamseed-plugin', 'plugin.json')
  ...
  const plugin: LoadedPlugin = {
    ...
    source, // Source identifier (e.g., "git:repo" or ".dreamseed-plugin/name")
 ...
 * main.tsx's getDreamSeedCodeMcpConfigs()/getInitialSettings().agent run
 ...
 * - After modifying .dreamseed-plugin/ directory (for export)
