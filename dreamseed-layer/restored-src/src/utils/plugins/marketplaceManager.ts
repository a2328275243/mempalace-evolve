/**
 * Marketplace manager for DreamSeed Code plugins
 *
 ...
 *
 * Reads the configuration file at ~/.dreamseed/plugins/known_marketplaces.json
 ...
 *     "installLocation": "/Users/me/.dreamseed/plugins/marketplaces/official-marketplace.json",
 *     "lastUpdated": "2024-01-15T10:30:00.000Z"
 *   },
 *   "company-plugins": {
 *     "source": { "source": "github", "repo": "mycompany/plugins" },
 *     "installLocation": "/Users/me/.dreamseed/plugins/marketplaces/company-plugins",
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a DreamSeed Code bug, shouldn't hit the error file.
    return {}
  }
}
 ...
 *
 * Writes the configuration to ~/.dreamseed/plugins/known_marketplaces.json,
 ...
 * - GitHub: Clones repo and looks for .dreamseed-plugin/marketplace.json
 ...
 *
 * Cache structure:
 * ~/.dreamseed/plugins/marketplaces/
    ...
    switch (source.source) {
      ...
      case 'github': {
        ...
        marketplacePath = join(
          temporaryCachePath,
          source.path || '.dreamseed-plugin/marketplace.json',
        )
        break
      }
      ...
      case 'file': {
        ...
        // File sources point to .dreamseed-plugin/marketplace.json, so the marketplace
        // root is two directories up (parent of .dreamseed-plugin/)
      ...
      case 'directory': {
        // For directories, look for .dreamseed-plugin/marketplace.json
        // Resolve to absolute so error messages show the actual path checked
        // (legacy known_marketplaces.json entries may have relative paths)
        const absPath = resolve(source.path)
        marketplacePath = join(absPath, '.dreamseed-plugin', 'marketplace.json')
 ...
 * The configuration is saved to ~/.dreamseed/plugins/known_marketplaces.json.
 *
...
async function readCachedMarketplace(
  ...
  // For git-sourced directories, the manifest lives at .dreamseed-plugin/marketplace.json.
  ...
  const nestedPath = join(installLocation, '.dreamseed-plugin', 'marketplace.json')
  try {
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `DreamSeed Code version. Run 'claude marketplace remove ${name}' and ` +
