...
function detectManifestType(
  ...
  // Check if it's in .dreamseed-plugin directory
  if (dirName === '.dreamseed-plugin') {
 ...
 * '..' a user added to "climb out of .dreamseed-plugin/" is unnecessary. Callers pass `hint`
...
// they expect paths to resolve relative to marketplace.json (inside .dreamseed-plugin/),
...
function marketplaceSourceHint(p: string): string {
  ...
  // .dreamseed-plugin/" is unnecessary since paths already start at the repo root.
  ...
  return (
    ...
    'containing .dreamseed-plugin/), not relative to marketplace.json. ' +
      ...
      for (const key of strayKeys) {
        ...
        warnings.push({
          ...
          message:
            ...
            `not plugin.json. It's harmless here but unused — DreamSeed Code ` +
  ...
  if (result.success) {
    ...
    if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(manifest.name)) {
      warnings.push({
        path: 'name',
        message:
          `Plugin name "${manifest.name}" is not kebab-case. DreamSeed Code accepts ` +
          `it, but the DreamSeed.ai marketplace sync requires kebab-case ` +
  ...
  if (result.success) {
    ...
    if (marketplace.plugins) {
      ...
      const marketplaceRoot =
        path.basename(manifestDir) === '.dreamseed-plugin'
      ...
      for (const [i, entry] of marketplace.plugins.entries()) {
        ...
        if (manifestVersion && manifestVersion !== entry.version) {
          warnings.push({
            ...
            message:
              `Entry declares version "${entry.version}" but ${entry.source}/.dreamseed-plugin/plugin.json says "${manifestVersion}". ` +
...
function validateComponentFile(
  ...
  if (fm.description !== undefined) {
    ...
    warnings.push({
      path: 'description',
      message:
        `No description in frontmatter. A description helps users and DreamSeed ` +
...
export async function validateManifest(
  ...
  if (stats?.isDirectory()) {
    // Look for manifest files in .dreamseed-plugin directory
    ...
    const pluginPath = path.join(absolutePath, '.dreamseed-plugin', 'plugin.json')
    ...
    return {
      ...
      errors: [
        {
          path: 'directory',
          message: `No manifest found in directory. Expected .dreamseed-plugin/marketplace.json or .dreamseed-plugin/plugin.json`,
        },
      ],
      warnings: [],
      filePath: absolutePath,
