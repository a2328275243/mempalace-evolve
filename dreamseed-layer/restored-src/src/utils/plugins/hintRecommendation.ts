 ...
 *
 * State persists in GlobalConfig.dreamseedCodeHints — a show-once record per
...
export function maybeRecordPluginHint(hint: ClaudeCodeHint): void {
  ...
  const state = getGlobalConfig().dreamseedCodeHints
...
export function markHintPluginShown(pluginId: string): void {
  saveGlobalConfig(current => {
    const existing = current.dreamseedCodeHints?.plugin ?? []
    if (existing.includes(pluginId)) return current
    return {
      ...current,
      claudeCodeHints: {
        ...current.dreamseedCodeHints,
...
export function disableHintRecommendations(): void {
  saveGlobalConfig(current => {
    if (current.dreamseedCodeHints?.disabled) return current
    return {
      ...current,
      claudeCodeHints: { ...current.dreamseedCodeHints, disabled: true },
    }
  })
}
