...
/**
 * `dreamseed ssh` remote: ANTHROPIC_UNIX_SOCKET routes auth through a -R forwarded
 * socket to a local proxy, and the launcher sets a handful of placeholder auth
 * env vars that the remote's ~/.dreamseed settings.env MUST NOT clobber (see
 ...
 * user's ~/.dreamseed/settings.json can't redirect requests away from the
 ...
 *
 * - userSettings (~/.dreamseed/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.dreamseed.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.dreamseed/settings.json env (gh#217). policy/flag
