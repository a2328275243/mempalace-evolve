...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.dreamseed/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.dreamseed/settings.json)'
    case 'projectSettings':
      return 'Project settings (.dreamseed/settings.json)'
    case 'localSettings':
      return 'Local settings (.dreamseed/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.dreamseed/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.dreamseed/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
      return 'Session hooks (in-memory, temporary)'
    case 'builtinHook':
      return 'Built-in hooks (registered internally by DreamSeed Code)'
    default:
