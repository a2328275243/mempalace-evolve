 ...
 * user-controlled files (~/.dreamseed/settings.json and ~/.dreamseed.json),
 ...
 *
 * We read from global config (~/.dreamseed.json) and user settings
 * (~/.dreamseed/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.dreamseed/settings.json),
