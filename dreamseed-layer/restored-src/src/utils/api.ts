...
export async function toolToAPISchema(
  ...
  if (!base) {
    ...
    // with DreamSeed 4.5 reject this field with 400. See GH#32742, PR #21729.
    if (
...
export async function logContextMetrics(
  ...
  const claudeMdSize = userContext.dreamseedMd?.length ?? 0
...
export function normalizeToolInput<T extends Tool>(
  ...
  switch (tool.name) {
    ...
    case BashTool.name: {
      ...
      // Logging for commands that are only echoing a string. This is to help us understand how often  DreamSeed talks via bash
