...
// hfi.tsx get a chance to set DREAMSEED_CONFIG_DIR in main(), and would also
...
export function isRunningFromLocalInstallation(): boolean {
  ...
  return execPath.includes('/.dreamseed/local/node_modules/')
}
  ...
  try {
    ...
    if (result.code !== 0) {
      const error = new Error(
        `Failed to install DreamSeed CLI package: ${result.stderr}`,
      )
      logError(error)
