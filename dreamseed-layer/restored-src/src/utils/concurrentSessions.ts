...
/**
 * Kind override from env. Set by the spawner (`dreamseed --bg`, daemon
...
/**
 * True when this REPL is running inside a `dreamseed --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `dreamseed ps` sees everything
...
export async function registerSession(): Promise<boolean> {
  ...
  try {
    ...
    // this, the PID file's sessionId goes stale and `dreamseed ps` sparkline
...
/**
 * Push live activity state for `dreamseed ps`. Fire-and-forget from REPL's
...
export async function countConcurrentSessions(): Promise<number> {
  ...
  for (const file of files) {
    ...
    if (isProcessRunning(pid)) {
      ...
      // ~/.dreamseed/sessions/ is shared with Windows-native DreamSeed (symlink
      // or DREAMSEED_CONFIG_DIR), a Windows PID won't be probeable from WSL
