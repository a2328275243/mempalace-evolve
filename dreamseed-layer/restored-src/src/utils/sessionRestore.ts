 ...
 * Used for computing initial state before render (per DREAMSEED.md guidelines).
...
export async function processResumedConversation(
  ...
  if (!opts.forkSession) {
    ...
    // the pre-loaded records). Without this seed, `dreamseed -r {newSessionId}`
  ...
  // Compute initial state before render (per DREAMSEED.md guidelines)
