...
/**
 * Resolve DreamSeed Code-specific path patterns for sandbox-runtime.
 *
 * DreamSeed Code uses special path prefixes in permission rules:
...
/**
 * Convert DreamSeed Code settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // Always include current directory and DreamSeed temp directory as writable
  ...
  // This blocks settings in the original working directory (where DreamSeed Code started)
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.dreamseed', 'settings.json'))
    denyWrite.push(resolve(cwd, '.dreamseed', 'settings.local.json'))
  }

  // Block writes to .dreamseed/skills in both original and current working directories.
  // The sandbox-runtime's getDangerousDirectories() protects .dreamseed/commands and
  // .dreamseed/agents but not .dreamseed/skills. Skills have the same privilege level
  // (auto-discovered, auto-loaded, full DreamSeed capabilities) so they need the
  // same OS-level sandbox protection.
  denyWrite.push(resolve(originalCwd, '.dreamseed', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.dreamseed', 'skills'))
  }

  // SECURITY: Git's is_git_directory() treats cwd as a bare repo if it has
  // HEAD + objects/ + refs/. An attacker planting these (plus a config with
  // core.fsmonitor) escapes the sandbox when DreamSeed's unsandboxed git runs.
  //
 ...
 * DreamSeed's unsandboxed git calls can see them. See the SECURITY block above
