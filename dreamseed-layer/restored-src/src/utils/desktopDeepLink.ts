...
/**
 * Builds a deep link URL for DreamSeed Desktop to resume a CLI session.
...
/**
 * Check if DreamSeed Desktop app is installed.
 * On macOS, checks for /Applications/DreamSeed.app.
...
async function isDesktopInstalled(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    // Check for DreamSeed.app in /Applications
...
/**
 * Detect the installed DreamSeed Desktop version.
...
/**
 * Build and open a deep link to resume the current session in DreamSeed Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open DreamSeed Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
