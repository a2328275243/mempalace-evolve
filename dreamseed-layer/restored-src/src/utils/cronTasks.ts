// Scheduled prompts, stored in <project>/.dreamseed/scheduled_tasks.json.
//
...
const CRON_FILE_REL = join('.dreamseed', 'scheduled_tasks.json')
...
/**
 * Read and parse .dreamseed/scheduled_tasks.json. Returns an empty task list if the file
...
/**
 * Overwrite .dreamseed/scheduled_tasks.json with the given tasks. Creates .dreamseed/ if
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.dreamseed'), { recursive: true })
 ...
 * written to .dreamseed/scheduled_tasks.json and dies with the process. The
 ...
 * and recurring tasks — a recurring task whose window passed while DreamSeed
