 ...
 *
 * This module provides PID-based locking for running DreamSeed Code versions.
...
/**
 * Validate that a running process is actually a DreamSeed process
...
export function isLockActive(lockFilePath: string): boolean {
  ...
  // Secondary validation: is it actually a DreamSeed process?
  // This helps with PID reuse scenarios
  if (!isClaudeProcess(pid, execPath)) {
    logForDebugging(
      `Lock PID ${pid} is running but does not appear to be DreamSeed - treating as stale`,
    )
    return false
  }
...
export async function tryAcquireLock(
  ...
  // validates it's actually a DreamSeed process (to handle PID reuse scenarios)
