...
export async function checkInstall(
  ...
  // isPossibleDreamSeedBinary stats the path internally, so we don't pre-check
  ...
  if (isWindows) {
    ...
    try {
      ...
      if (!(await isPossibleClaudeBinary(absoluteTarget))) {
        messages.push({
          message: `DreamSeed symlink points to missing or invalid binary: ${target}`,
          userActionRequired: true,
          type: 'error',
        })
      }
      ...
      if (isENOENT(e)) {
        ...
        if (!(await isPossibleClaudeBinary(dirs.executable))) {
          messages.push({
            message: `${dirs.executable} exists but is not a valid DreamSeed binary`,
            userActionRequired: true,
            type: 'error',
          })
        }
...
export async function cleanupNpmInstallations(): Promise<{
  ...
  // Check for local installation at ~/.dreamseed/local
  const localInstallDir = join(homedir(), '.dreamseed', 'local')
