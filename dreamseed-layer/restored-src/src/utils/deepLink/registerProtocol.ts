...
export const MACOS_BUNDLE_ID = 'com.anthropic.dreamseed-code-url-handler'
const APP_NAME = 'DreamSeed Code URL Handler'
const DESKTOP_FILE_NAME = 'claude-code-url-handler.desktop'
const MACOS_APP_NAME = 'DreamSeed Code URL Handler.app'
 ...
 * DreamSeed then uses the url-handler NAPI module to read the URL from the
 ...
 * a cached flag in ~/.dreamseed.json, so:
...
export async function ensureDeepLinkProtocolRegistered(): Promise<void> {
  ...
  // ~/.dreamseed (per-machine, not synced) rather than ~/.dreamseed.json (can sync).
