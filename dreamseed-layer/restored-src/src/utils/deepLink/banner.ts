 ...
 * notice which directory — and therefore which DREAMSEED.md — was loaded.
 */
 ...
 *
 * Always shows the working directory so the user can see which DREAMSEED.md
 ...
 * and whether its DREAMSEED.md may be stale relative to upstream.
 */
export function buildDeepLinkBanner(info: DeepLinkBannerInfo): string {
  ...
  if (info.repo) {
    ...
    lines.push(
      `Resolved ${info.repo} from local clones · last fetched ${age}${stale ? ' — DREAMSEED.md may be stale' : ''}`,
    )
  }
