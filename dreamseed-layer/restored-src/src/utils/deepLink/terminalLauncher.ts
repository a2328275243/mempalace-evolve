 ...
 * Terminal Launcher
 *
 * Detects the user's preferred terminal emulator and launches DreamSeed Code
...
/**
 * Launch DreamSeed Code in the detected terminal emulator.
 *
...
async function launchMacosTerminal(
  ...
  activate
    ...
    case 'Ghostty': {
      ...
      args.push('-e', claudePath, ...dreamseedArgs)
    ...
    case 'kitty': {
      ...
      args.push(claudePath, ...dreamseedArgs)
      const { code } = await execFileNoThrow('open', args, { useCwd: false })
      if (code === 0) return true
      break
    }
...
async function launchLinuxTerminal(
  ...
  switch (terminal.name) {
    ...
    default:
      ...
      args = ['-e', claudePath, ...dreamseedArgs]
      spawnCwd = cwd
      break
  }
...
function buildShellCommand(
  ...
  return `${cdPrefix}${[claudePath, ...dreamseedArgs].map(shellQuote).join(' ')}`
}
