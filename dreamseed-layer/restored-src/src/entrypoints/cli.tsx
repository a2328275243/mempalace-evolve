...
if (feature('ABLATION_BASELINE') && process.env.CLAUDE_CODE_ABLATION_BASELINE) {
  for (const k of ['CLAUDE_CODE_SIMPLE', 'CLAUDE_CODE_DISABLE_THINKING', 'DISABLE_INTERLEAVED_THINKING', 'DISABLE_COMPACT', 'DISABLE_AUTO_COMPACT', 'DREAMSEED_DISABLE_AUTO_MEMORY', 'CLAUDE_CODE_DISABLE_BACKGROUND_TASKS']) {
    // eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
    process.env[k] ??= '1';
  }
}
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (DreamSeed Code)`);
    return;
  }
  ...
  // Fast-path for `dreamseed remote-control` (also accepts legacy `dreamseed remote` / `dreamseed sync` / `dreamseed bridge`):
  ...
  // Fast-path for `dreamseed daemon [subcommand]`: long-running supervisor.
  ...
  // Fast-path for `dreamseed ps|logs|attach|kill` and `--bg`/`--background`.
  // Session management against the ~/.dreamseed/sessions/ registry. Flag
  ...
  // Fast-path for `dreamseed environment-runner`: headless BYOC runner.
  // feature() must stay inline for build-time dead code elimination.
  ...
  // Fast-path for `dreamseed self-hosted-runner`: headless self-hosted-runner
