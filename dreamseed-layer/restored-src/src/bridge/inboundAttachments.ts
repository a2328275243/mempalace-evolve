 ...
 * (oauth-authed, same store), write to ~/.dreamseed/uploads/{sessionId}/, and
 * return @path refs to prepend. DreamSeed's Read tool takes it from there.
 *
 * Best-effort: any failure (no token, network, non-2xx, disk) logs debug and
 * skips that attachment. The message still reaches DreamSeed, just without @path.
 */
