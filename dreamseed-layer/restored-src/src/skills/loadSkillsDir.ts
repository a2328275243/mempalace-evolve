...
export function getSkillsPath(
  ...
  switch (source) {
    case 'policySettings':
      return join(getManagedFilePath(), '.dreamseed', dir)
    case 'userSettings':
      return join(getClaudeConfigHomeDir(), dir)
    case 'projectSettings':
      return `.dreamseed/${dir}`
    case 'plugin':
      return 'plugin'
    default:
      return ''
...
/**
 * Parse paths frontmatter from a skill, using the same format as DREAMSEED.md rules.
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    ...
    const managedSkillsDir = join(getManagedFilePath(), '.dreamseed', 'skills')
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      const skillDir = join(currentDir, '.dreamseed', 'skills')
      ...
      if (!dynamicSkillDirs.has(skillDir)) {
        ...
        try {
          ...
          // is gitignored — blocks e.g. node_modules/pkg/.dreamseed/skills from
 ...
 * of DREAMSEED.md conditional rules.
 *
