 ...
 * `dreamseed mcp add` expressed higher intent than a connector toggled in the web UI.
 *
 ...
 * in getDreamSeedCodeMcpConfigs(): --mcp-config (main.tsx) and the mcp_set_servers
...
export function getMcpConfigByName(name: string): ScopedMcpServerConfig | null {
  ...
  // by name. User/project/local servers are blocked — same as getDreamSeedCodeMcpConfigs().
 ...
 * @returns DreamSeed Code server configurations with appropriate scopes
 */
 ...
 * This may be slow due to network calls - use getDreamSeedCodeMcpConfigs() for fast startup.
  ...
  for (const [name, config] of Object.entries(schemaResult.data.mcpServers)) {
    ...
    if (
      ...
      errors.push({
        ...
        suggestion: `Change command to "cmd" with args ["/c", "npx", ...]. See: https://code.dreamseed.com/docs/en/mcp#configure-mcp-servers`,
        mcpErrorMetadata: {
          scope,
          serverName: name,
          severity: 'warning',
