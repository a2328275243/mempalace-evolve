...
export const fetchClaudeAIMcpConfigsIfEligible = memoize(
  async (): Promise<Record<string, ScopedMcpServerConfig>> => {
    try {
      ...
      // Check for user:mcp_servers scope directly instead of isDreamSeedAISubscriber().
...
/**
 * Clears the memoized cache for fetchDreamSeedAIMcpConfigsIfEligible.
...
export function markClaudeAiMcpConnected(name: string): void {
  saveGlobalConfig(current => {
    const seen = current.dreamseedAiMcpEverConnected ?? []
...
export function hasClaudeAiMcpEverConnected(name: string): boolean {
  return (getGlobalConfig().dreamseedAiMcpEverConnected ?? []).includes(name)
}
