 ...
 * Used by DreamSeedAuthProvider for metadata discovery and token refresh.
 ...
 *
 * Per RFC 7009, public clients (like DreamSeed Code) should authenticate by including
 ...
 * `dreamseed mcp xaa setup`). Per-server config is just `oauth.xaa: true`
...
async function performMCPXaaAuth(
  ...
  if (!idp) {
    throw new Error(
      "XAA: no IdP connection configured. Run 'dreamseed mcp xaa setup --issuer <url> --client-id <id> --client-secret' to configure.",
    )
  }
  ...
  try {
    ...
    // (instead of DreamSeedAuthProvider.saveTokens) to avoid instantiating the
...
export async function performMCPOAuthFlow(
  ...
  // rest of CC's transport wiring (DreamSeedAuthProvider.tokens() in client.ts)
  // works unchanged.
  //
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          if (code) {
            res.writeHead(200, { 'Content-Type': 'text/html' })
            res.end(
              `<h1>Authentication Successful</h1><p>You can close this window. Return to DreamSeed Code.</p>`,
            )
            cleanup()
            resolveOnce(code)
          }
...
export class ClaudeAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `DreamSeed Code (${this.serverName})`,
   ...
   * but this is the shape DREAMSEED.md flags under "Token/auth caching across
