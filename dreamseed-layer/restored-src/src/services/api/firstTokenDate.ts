...
/**
 * Fetch the user's first DreamSeed Code token date and store in config.
 * This is called after successful login to cache when they started using DreamSeed Code.
 */
export async function fetchAndStoreClaudeCodeFirstTokenDate(): Promise<void> {
  try {
    const config = getGlobalConfig()

    if (config.dreamseedCodeFirstTokenDate !== undefined) {
      return
    }
