...
export function Doctor(t0) {
  ...
  if ($[6] !== agentDefinitions || $[7] !== toolPermissionContext || $[8] !== tools) {
    t5 = () => {
      ...
      (async () => {
        ...
        const projectAgentsDir = join(getOriginalCwd(), ".dreamseed", "agents");
        const {
          activeAgents,
          allAgents,
          failedFiles
  ...
  if ($[11] !== onDone) {
    t7 = () => {
      onDone("DreamSeed Code diagnostics dismissed", {
        display: "system"
      });
    };
    $[11] = onDone;
  ...
  if ($[73] !== contextWarnings) {
    t39 = contextWarnings && (contextWarnings.dreamseedMdWarning || contextWarnings.agentWarning || contextWarnings.mcpWarning) && <Box flexDirection="column"><Text bold={true}>Context Usage Warnings</Text>{contextWarnings.dreamseedMdWarning && <><Text>└{" "}<Text color="warning">{figures.warning} {contextWarnings.dreamseedMdWarning.message}</Text></Text><Text>{"  "}└ Files:</Text>{contextWarnings.dreamseedMdWarning.details.map(_temp16)}</>}{contextWarnings.agentWarning && <><Text>└{" "}<Text color="warning">{figures.warning} {contextWarnings.agentWarning.message}</Text></Text><Text>{"  "}└ Top contributors:</Text>{contextWarnings.agentWarning.details.map(_temp17)}</>}{contextWarnings.mcpWarning && <><Text>└{" "}<Text color="warning">{figures.warning} {contextWarnings.mcpWarning.message}</Text></Text><Text>{"  "}└ MCP servers:</Text>{contextWarnings.mcpWarning.details.map(_temp18)}</>}</Box>;
    $[73] = contextWarnings;
    $[74] = t39;
  } else {
    t39 = $[74];
