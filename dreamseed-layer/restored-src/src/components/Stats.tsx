...
function StatsContent(t0) {
  ...
  if (allTimeResult.type === "empty") {
    ...
    if ($[15] === Symbol.for("react.memo_cache_sentinel")) {
      t7 = <Box marginTop={1}><Text color="warning">No stats available yet. Start using DreamSeed Code!</Text></Box>;
      $[15] = t7;
    } else {
      t7 = $[15];
    }
...
async function handleScreenshot(stats: DreamSeedCodeStats, activeTab: 'Overview' | 'Models', setStatus: (status: string | null) => void): Promise<void> {
  setStatus('copying…');
...
function renderStatsToAnsi(stats: DreamSeedCodeStats, activeTab: 'Overview' | 'Models'): string {
  const lines: string[] = [];
...
function renderOverviewToAnsi(stats: ClaudeCodeStats): string[] {
  const lines: string[] = [];
  const theme = getTheme(resolveThemeSetting(getGlobalConfig().theme));
  const h = (text: string) => applyColor(text, theme.dreamseed as Color);
