 ...
 *   1. DREAMSEED_DISABLE_AUTO_MEMORY env var (1/true → OFF, 0/false → ON)
 *   2. CLAUDE_CODE_SIMPLE (--bare) → OFF
 *   3. CCR without persistent storage → OFF (no DREAMSEED_REMOTE_MEMORY_DIR)
 *   4. autoMemoryEnabled in settings.json (supports project-level opt-out)
 *   5. Default: enabled
 */
export function isAutoMemoryEnabled(): boolean {
  const envVal = process.env.DREAMSEED_DISABLE_AUTO_MEMORY
 ...
 * Resolution order:
 *   1. DREAMSEED_REMOTE_MEMORY_DIR env var (explicit override, set in CCR)
 *   2. ~/.dreamseed (default config home)
 */
export function getMemoryBaseDir(): string {
  if (process.env.DREAMSEED_REMOTE_MEMORY_DIR) {
    return process.env.DREAMSEED_REMOTE_MEMORY_DIR
  }
...
function getAutoMemPathOverride(): string | undefined {
  return validateMemoryPath(
    process.env.DREAMSEED_COWORK_MEMORY_PATH_OVERRIDE,
    false,
  )
}
 ...
 *
 * SECURITY: projectSettings (.dreamseed/settings.json committed to the repo) is
...
/**
 * Check if DREAMSEED_COWORK_MEMORY_PATH_OVERRIDE is set to a valid override.
 ...
 *
 * Resolution order:
 *   1. DREAMSEED_COWORK_MEMORY_PATH_OVERRIDE env var (full-path override, used by Cowork)
 ...
 * env vars / settings.json / DREAMSEED_CONFIG_DIR are session-stable in
 ...
 *
 * When DREAMSEED_COWORK_MEMORY_PATH_OVERRIDE is set, this matches against the
