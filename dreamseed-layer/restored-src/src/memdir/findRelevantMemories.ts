...
const SELECT_MEMORIES_SYSTEM_PROMPT = `You are selecting memories that will be useful to DreamSeed Code as it processes a user's query. You will be given the user's query and a list of available memory files with their filenames and descriptions.

Return a list of filenames for the memories that will clearly be useful to DreamSeed Code as it processes the user's query (up to 5). Only include memories that you are certain will be helpful based on their name and description.
...
async function selectRelevantMemories(
  ...
  // When DreamSeed Code is actively using a tool (e.g. mcp__X__spawn),
