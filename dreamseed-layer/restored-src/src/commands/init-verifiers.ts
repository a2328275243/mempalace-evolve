...
3. **Installed verification packages** (for web apps)
   ...
   - Check MCP configuration (.mcp.json) for browser automation tools:
     ...
     - DreamSeed Browser Extension MCP (browser-use via DreamSeed's Chrome extension)
...
**All verifier skills are created in the project root's \`.dreamseed/skills/\` directory.** This ensures they are automatically loaded when DreamSeed runs in the project.

Write the skill file to \`.dreamseed/skills/<verifier-name>/SKILL.md\`.
...
1. Where each skill was created (always in \`.dreamseed/skills/\`)
