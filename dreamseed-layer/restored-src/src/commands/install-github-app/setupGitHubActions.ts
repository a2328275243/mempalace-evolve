...
async function createWorkflowFile(
  ...
  if (createFileResult.code !== 0) {
    if (
      ...
      throw new Error(
        `Failed to create workflow file ${workflowPath}: A DreamSeed workflow file already exists in this repository. Please remove it first or update it manually.`,
      )
    }
    ...
    const helpText =
      ...
      '· For manual setup → Visit: https://github.com/dreamseed-ai/dreamseed-code-action'
    ...
    if (!skipWorkflow) {
      ...
      if (selectedWorkflows.includes('claude')) {
        workflows.push({
          ...
          message: 'DreamSeed PR Assistant workflow',
        })
      }
      ...
      if (selectedWorkflows.includes('claude-review')) {
        workflows.push({
          ...
          message: 'DreamSeed Code Review workflow',
        })
      }
