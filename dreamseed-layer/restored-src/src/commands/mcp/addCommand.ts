...
export function registerMcpAddCommand(mcp: Command): void {
  mcp
    ...
    .description(
      'Add an MCP server to DreamSeed Code.\n\n' +
        'Examples:\n' +
        '  # Add HTTP server:\n' +
        ...
        '  dreamseed mcp add -e API_KEY=xxx my-server -- npx my-mcp-server\n\n' +
        '  # Add stdio server with subprocess flags:\n' +
        '  dreamseed mcp add my-server -- my-command --some-flag arg1',
    )
    .option(
      '-s, --scope <scope>',
    ...
    .addOption(
      new Option(
        '--xaa',
        "Enable XAA (SEP-990) for this server. Requires 'dreamseed mcp xaa setup' first. Also requires --client-id and --client-secret (for the MCP server's AS).",
      ...
      if (!name) {
        cliError(
          'Error: Server name is required.\n' +
            'Usage: dreamseed mcp add <name> <command> [args...]',
        )
      ...
      try {
        ...
        if (xaa) {
          ...
          if (!getXaaIdpSettings()) {
            missing.push(
              "'dreamseed mcp xaa setup' (settings.xaaIdp not configured)",
            )
          }
          if (missing.length) {
        ...
        if (transport === 'sse') {
          ...
          if (!transportExplicit && looksLikeUrl) {
            ...
            process.stderr.write(
              `If this is an HTTP server, use: dreamseed mcp add --transport http ${name} ${actualCommand}\n`,
            )
            process.stderr.write(
              `If this is an SSE server, use: dreamseed mcp add --transport sse ${name} ${actualCommand}\n`,
            )
          }
