...
def main() -> int:
    ...
    for name in ("build", "status", "show"):
        ...
        p.add_argument("--local-root", default=os.environ.get("DREAMSEED_LOCAL_ROOT", str(Path.home() / ".dreamseed-local")))
    ...
    if not args.command:
        args.command = "status"
        args.root = str(ROOT)
        args.local_root = os.environ.get("DREAMSEED_LOCAL_ROOT", str(Path.home() / ".dreamseed-local"))
        args.max_files = 160
        args.json = False
...
def build_cache(root: Path, local_root: Path, max_files: int) -> dict[str, Any]:
    # Incremental: load existing cache, skip unchanged files by mtime+size
    existing = read_cache(root, local_root)
    existing_files = {}
    if existing.get("projects"):
        for p in existing["projects"]:
            for s in p.get("recentSummaries", []):
                pass
    existing_mtime = {}
    if existing.get("_fileIndex"):
        existing_mtime = existing["_fileIndex"]
