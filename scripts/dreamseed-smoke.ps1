...
function Resolve-DreamSeedCommand {
  foreach ($LocalLauncher in @(
    (Join-Path (Split-Path -Parent (Split-Path -Parent $RepoRoot)) "bin\dreamseed-local.cmd"),
    (Join-Path (Split-Path -Parent (Split-Path -Parent $RepoRoot)) "bin\dreamseed.cmd")
  )) {
...
function ConvertFrom-MixedJsonOutput {
  ...
  throw "No parseable JSON object found in output: $($Text.Substring([Math]::Max(0, $Text.Length - 1200)))"
}
...
$LocalBin = Join-Path (Split-Path -Parent (Split-Path -Parent $RepoRoot)) "bin"
if (Test-Path -LiteralPath $LocalBin) {
  $env:PATH = "$LocalBin;$env:PATH"
}
$env:DREAMSEED_DESKTOP_SMOKE_SOURCE = "1"
