# release-exclusions: __pycache__ *.pyc self-evolve-candidates self-evolve-backups logs cache providers.local.json history.meta.json desktop.local.json legacy-history memory-candidates
param(
  [string]$Version = "",
...
# Plan B cleanup: drop runtime data accidentally captured under .dreamseed/ in the dev tree.
$RuntimeRubbish = @(".dreamseed\cache", ".dreamseed\history", ".dreamseed\logs", ".dreamseed\state")
foreach ($Rel in $RuntimeRubbish) {
