...
def compute_content_hash(text: str) -> str:
    """Normalized content hash for dedup detection."""
    import hashlib, re
    normalized = re.sub(r"\s+", " ", text.strip().lower())
    return hashlib.sha256(normalized.encode("utf-8")).hexdigest()[:16]

def check_duplicate(text: str, candidate_dir) -> dict | None:
    """Check if similar content already exists among candidates."""
    import hashlib, re
    target = re.sub(r"\s+", " ", text.strip().lower())[:200]
    ...
    for f in candidate_dir.glob("*.json"):
        try:
            ...
            normalized = re.sub(r"\s+", " ", existing.strip().lower())[:200]
            if normalized == target:
                return {"file": str(f), "score": data.get("score", 0)}
        except Exception:
            continue
    return None
...
def apply_candidate(path: Path, min_score: float = 0.55) -> dict[str, Any]:
    ...
    text_for_dedup = candidate_text(data)
    dup = check_duplicate(text_for_dedup, REVIEWED_DIR)
    if dup:
        return move_with_review(path, REJECTED_DIR, "rejected", "duplicate of existing reviewed candidate", decision_data)
