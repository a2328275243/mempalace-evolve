...
USER_INTERACTION_TOOL_RE = re.compile(
    r"(?i)(ask|question|prompt|choice|confirm|request[_-]?user[_-]?input|user[_-]?input|exit[_-]?plan|plan[_-]?mode)"
)
...
def run_hook(policy: dict[str, Any], hook_input_path: str = "") -> int:
    ...
    if result["decision"] == "pass":
        # PermissionRequest hooks defer to the native UI only when stdout is
        # completely empty. Even {"suppressOutput": true} is treated as a hook
        # response by the compatible kernel and can swallow user-choice tools.
        return 0
    ...
    if result["decision"] == "ask" and desktop_approval_enabled():
        desktop_result = request_desktop_approval(payload, result)
        if desktop_result.get("approved"):
            print_json(
                {
                    "suppressOutput": True,
                    "hookSpecificOutput": {
        ...
        print_json(
            {
                ...
                "hookSpecificOutput": {
                    ...
                    "decision": {
                        "behavior": "deny",
                        "message": desktop_result.get("message")
                        or "DreamSeed desktop approval was denied or timed out.",
                    },
                },
            }
        )
    ...
    # No decision means the native permission prompt asks the user. Keep stdout
    # empty here too; printing JSON changes the hook outcome.
    return 0
...
def classify_hook_payload(policy: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    return classify_tool_request(policy, extract_tool_name(payload), extract_tool_input(payload))
...
def classify_tool_request(policy: dict[str, Any], tool_name: str, tool_input: Any) -> dict[str, Any]:
    ...
    if tool.lower() in {"bash", "shell"} and command:
    ...
    elif is_user_interaction_tool(tool):
        risk = "low"
        reasons.append(f"{tool} is a user interaction request; pass through to native UI")
        tags.append("user-interaction")
        ...
        risk = "high"
        reasons.append(f"{tool} is not recognized; fall through to the native permission or interaction UI")
    ...
    decision = "pass" if "user-interaction" in tags else decision_for_risk(policy, risk)
    return {
        "ok": True,
        "tool": tool,
        "risk": risk,
        ...
        "inputPreview": input_preview(tool_input),
    }
...
def is_user_interaction_tool(name: str) -> bool:
    normalized = str(name or "").strip()
    return bool(USER_INTERACTION_TOOL_RE.search(normalized))
...
def audit_report(policy: dict[str, Any]) -> dict[str, Any]:
    ...
    samples = [
        ...
        ("AskUserQuestion", {"question": "Continue?", "choices": ["Yes", "No"]}, "pass"),
        ("unknown", {}, "ask"),
    ]
    sample_results = []
...
def desktop_approval_enabled() -> bool:
    return bool(os.environ.get("DREAMSEED_DESKTOP_APPROVAL_DIR"))


def request_desktop_approval(payload: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    directory = Path(os.environ.get("DREAMSEED_DESKTOP_APPROVAL_DIR", ""))
    if not directory:
        return {"approved": False, "message": "DreamSeed desktop approval directory is not configured."}
    timeout_ms = int(os.environ.get("DREAMSEED_DESKTOP_APPROVAL_TIMEOUT_MS") or "120000")
    timeout_ms = max(5000, min(timeout_ms, 600000))
    ...
    try:
        ...
        request = {
            ...
            "expiresAt": datetime.fromtimestamp((time.time() + timeout_ms / 1000), timezone.utc).isoformat(),
            "pid": os.getpid(),
            "tool": result.get("tool"),
            "risk": result.get("risk"),
            "decision": result.get("decision"),
            ...
            "commandPreview": result.get("commandPreview", ""),
            "inputPreview": result.get("inputPreview", ""),
            "taskId": os.environ.get("DREAMSEED_DESKTOP_TASK_ID", ""),
            "cwd": os.getcwd(),
        }
        ...
        while time.time() < deadline:
            if response_path.exists():
                ...
                cleanup_desktop_approval_files(request_path, response_path)
                return {
                    "approved": approved,
                    "message": "DreamSeed desktop approval granted." if approved else "DreamSeed desktop approval denied.",
                }
            time.sleep(0.2)
        cleanup_desktop_approval_files(request_path, response_path)
        return {"approved": False, "message": "DreamSeed desktop approval timed out."}
    except Exception as exc:
        cleanup_desktop_approval_files(request_path, response_path)
        return {"approved": False, "message": f"DreamSeed desktop approval failed: {exc}"}


def cleanup_desktop_approval_files(*paths: Path) -> None:
    for path in paths:
        try:
            if path.exists():
                path.unlink()
